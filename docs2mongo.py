...
try:
    test = site_coll.find_one({'name': 'test'})
except TypeError:
    site_coll.find_one = site_coll.find
    site_coll.insert_one = site_coll.insert
    site_coll.update_one = site_coll.update
    site_coll.find_one_and_delete = site_coll.find_and_delete
...
def update_settings(site_name, settings_dict):
    ...
    """
    return site_coll.update_one({'name': site_name},
...
def _add_doc(doc):
    return site_coll.insert_one(doc).inserted_id
